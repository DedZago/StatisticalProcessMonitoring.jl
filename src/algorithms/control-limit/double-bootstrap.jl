...
function approximateBisectionCL!(CH::ControlChart; rlsim::Function = run_path_sim, maxiter::Int = 30, nsims::Int = 1000, maxrl::Real = Int(min(get_maxrl(CH), 10*get_nominal_value(CH))), x_tol::Float64 = 1e-06, f_tol::Float64 = 1.0, B::Int = nsims, verbose::Bool = false, parallel::Bool = false)
    ...
    error("Generalize rl_paths to a function to obtain matrix/tensor as needed")
    rl_paths = _calculate_rl_paths(CH, nsims_i, maxrl_i, parallel)
...
function _calculate_rl_paths(CH::ControlChart, nsims_i, maxrl_i, parallel)
...
function _calculate_rl_paths(CH::MultipleControlChart, nsims_i, maxrl_i, parallel)
    error("Not implemented yet")
    rl_paths = Matrix{Float64}(undef, nsims_i, maxrl_i, length(get_statistic(CH)))    # Generated run length paths
    if parallel
        Threads.@threads for i in 1:nsims_i
            rl_paths[i, :] = rlsim(CH, maxiter = maxrl)
        end
